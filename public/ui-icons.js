...
(function () {
  ...
  const RAW_MAP = {
    ...
    // ── NEW in v7 — Chat page cleanup: header/menu/toolbar controls that
  ...
  const SAFE_SELECTORS = [
    ...
    // ── Chat page — menu/sheet/context-menu rows and option lists. These
    // are fixed UI-control labels (icon + static text) generated by chat.js
    // and the wallpaper modal, never user-typed content, so they're safe
    // to convert the same as any other menu row elsewhere in the app.
    // NOTE: '.chat-sheet-opt' is intentionally scoped to the attach-menu
    // (#chatBottomSheet) only — the same class is reused by the Couple
    // Gifts panel (#chatGiftPanel), where the emoji (🌹💐🍫 etc.) is the
    // actual gift artwork being picked, not a UI icon, and must stay as-is
    // (same reason stickers/mood/reactions elsewhere are excluded).
    '.chat-header-menu-item', '.wp-option-row', '.ctx-item',
