...
const HeartbeatManager = (() => {
  ...
  function hookIntoApp() {
    ...
    /* Hook sendChat() to trigger particles & typing presence */
    const _origSendChat = window.sendChat;
    if (_origSendChat) {
      window.sendChat = function() {
        _origSendChat();
        HeartbeatManager.onMessageSent();
      };
    }

    /* Hook chat input typing */
    const chatIn = document.getElementById('chatIn');
    if (chatIn) {
      chatIn.addEventListener('input', () => HeartbeatManager.onChatTyping(), { passive: true });
    }
